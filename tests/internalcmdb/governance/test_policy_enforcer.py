...
from internalcmdb.governance.policy_enforcer import (
    PolicyCheckResult,
    PolicyEnforcer,
    PolicyViolation,
)
...
def test_blocked_action_violation():
    r = PolicyEnforcer(_db([_pol(rules={"blocked_actions": ["delete"]})])).check(
        {"type": "delete", "target": "h"}
    )
    assert not r.compliant
    assert "blocked" in r.violations[0].reason.lower()
...
def test_requires_approval_no_approval():
    r = PolicyEnforcer(_db([_pol(rules={"requires_approval_for": ["deploy"]})])).check(
        {"type": "deploy", "target": "p"}
    )
    assert not r.compliant
    assert any("approval" in v.reason.lower() for v in r.violations)


def test_requires_approval_with_approval():
    r = PolicyEnforcer(_db([_pol(rules={"requires_approval_for": ["deploy"]})])).check(
        {"type": "deploy", "target": "p", "has_approval": True}
    )
    assert r.compliant


def test_restricted_target_blocks():
    r = PolicyEnforcer(_db([_pol(rules={"restricted_targets": ["prod"]})])).check(
        {"type": "update", "target": "prod"}
    )
    assert not r.compliant
    assert any("restricted" in v.reason.lower() for v in r.violations)


def test_restricted_target_with_override():
    r = PolicyEnforcer(_db([_pol(rules={"restricted_targets": ["prod"]})])).check(
        {"type": "update", "target": "prod", "override": True}
    )
    assert r.compliant
...
def test_db_failure_fail_closed():
    ...
    assert not r.compliant
    assert r.violations[0].policy_code == "SYS-FAIL-CLOSED"
...
def test_policy_check_result_violations_as_tuple():
    ...
    assert r.violations[0].policy_code == "P1"
