...
def test_setup_tracing_importable():
    from internalcmdb.observability import tracing  # noqa: PLC0415
...
def test_record_llm_span_attributes_with_mock_span():
    ...
    record_llm_span_attributes(
        mock_span,
        LlmSpanAttrs(model="test-model", input_tokens=10, output_tokens=20),
    )
...
def test_record_llm_span_attributes_none_span():
    ...
    record_llm_span_attributes(
        None, LlmSpanAttrs(model="test-model", input_tokens=0, output_tokens=0)
    )
...
def test_record_llm_span_attributes_finish_reasons():
    ...
    mock_span = MagicMock()
    record_llm_span_attributes(mock_span, LlmSpanAttrs(model="test", finish_reasons=["stop"]))
