...
async def test_gather_return_exceptions_does_not_propagate() -> None:
    ...
    results = await asyncio.gather(_fail(), _cancel(), return_exceptions=True)
...
def test_main_py_uses_gather_not_try_except() -> None:
    ...
    content = (_project_root / "src" / "internalcmdb" / "api" / "main.py").read_text()
    ...
    cancelled_catches = [
        ...
        if "except asyncio.CancelledError" in line or "except CancelledError" in line
    ]
