...
def test_llm_backend_defaults_contains_all_four_backends() -> None:
    from internalcmdb.api.routers.settings import _LLM_BACKEND_DEFAULTS  # noqa: PLC0415
...
def test_llm_backend_defaults_each_entry_is_url_modelid_timeout() -> None:
    ...
    for name, (url, model_id, timeout_s) in _LLM_BACKEND_DEFAULTS.items():
        assert isinstance(url, str), f"Backend '{name}' default URL {url!r} must be a string"
        assert url.startswith("http"), f"Backend '{name}' default URL {url!r} must be an HTTP URL"
        assert isinstance(model_id, str), f"Backend '{name}' default model_id must be a string"
        assert model_id, f"Backend '{name}' default model_id must be a non-empty string"
        assert isinstance(timeout_s, int), f"Backend '{name}' default timeout_s must be an integer"
        assert timeout_s > 0, f"Backend '{name}' default timeout_s must be a positive integer"
...
def test_sk_guard_url_constant() -> None:
    from internalcmdb.api.routers.settings import _SK_GUARD_URL  # noqa: PLC0415

    assert _SK_GUARD_URL == "llm.guard.url"


def test_sk_obs_debug_constant() -> None:
    from internalcmdb.api.routers.settings import _SK_OBS_DEBUG  # noqa: PLC0415

    assert _SK_OBS_DEBUG == "obs.debug_enabled"


def test_msg_channel_not_found_constant() -> None:
    from internalcmdb.api.routers.settings import _MSG_CHANNEL_NOT_FOUND  # noqa: PLC0415
...
def _make_store_returning(values: dict[str, object]) -> AsyncMock:
    ...
    store = AsyncMock()
    store.get = AsyncMock(side_effect=values.get)
    return store


@pytest.mark.asyncio
async def test_resolve_llm_backend_returns_defaults_when_store_empty() -> None:
    from internalcmdb.api.routers.settings import (  # noqa: PLC0415
...
async def test_resolve_llm_backend_uses_store_values_when_present() -> None:
    from internalcmdb.api.routers.settings import _resolve_llm_backend  # noqa: PLC0415
