...
def test_missing_entity_id():
    ...
    assert not r.has_drift and r.drift_type == "error"
...
def test_canonical_none():
    ...
    assert r.has_drift and r.drift_type == "missing_canonical" and "cpu" in r.fields_changed


def test_canonical_empty():
    r = _det().detect_drift("h1", {"cpu": "10"}, {})
    assert r.has_drift and r.drift_type == "missing_canonical"


def test_no_drift():
    r = _det().detect_drift("h1", {"version": "1.0"}, {"version": "1.0"})
    assert not r.has_drift and r.fields_changed == []
...
def test_intentional_drift():
    r = _det().detect_drift("h1", {"version": "2.0", "image": "nginx:2.0"}, {"version": "1.0", "image": "nginx:1.0"})
    assert r.has_drift and r.drift_type == "intentional"


def test_accidental_drift():
    r = _det().detect_drift("h1", {"custom_field": "new"}, {"custom_field": "old"})
    assert r.has_drift and r.drift_type == "accidental"


def test_critical_drift_firewall():
    r = _det().detect_drift("h1", {"firewall_rules": "DROP ALL"}, {"firewall_rules": "ACCEPT"})
    assert r.has_drift and r.drift_type == "critical"
...
def test_critical_overrides_intentional():
    r = _det().detect_drift("h1", {"version": "2.0", "root_login": "yes"}, {"version": "1.0", "root_login": "no"})
    assert r.drift_type == "critical"


def test_confidence_between_zero_and_one():
    r = _det().detect_drift("h1", {"firewall_rules": "a", "version": "2"}, {"firewall_rules": "b", "version": "1"})
    assert 0.0 <= r.confidence <= 1.0


def test_explanation_present():
    r = _det().detect_drift("h1", {"version": "2"}, {"version": "1"})
    assert r.explanation and "h1" in r.explanation
