...
/** Jobs in a terminal state can be retried. */
const canRetry = (status: string) => status === "failed" || status === "completed";
/** Jobs in an active state can be cancelled. */
  ...
  if (isLoading)
    return (
      <div className="space-y-2">
        {(["sk-a","sk-b","sk-c","sk-d","sk-e","sk-f","sk-g","sk-h"] as const).map((key) => (
          ...
          <TableBody>
            {data?.items.map((job) => {
              ...
              if (job.started_at && job.finished_at) {
                dur = `${((new Date(job.finished_at).getTime() - new Date(job.started_at).getTime()) / 1000).toFixed(1)}s`;
              ...
              return (
                <TableRow key={job.job_id}>
                  ...
                  <TableCell className="text-xs text-(--tx3) whitespace-nowrap" title={formatDate(job.started_at)}>
