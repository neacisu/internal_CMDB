...
def _get_tool_engine():  # noqa: ANN202
...
def _sync_query_fleet_summary(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        with engine.connect() as conn:
            ...
            agents = conn.execute(
                text("SELECT COUNT(*) FROM discovery.collector_agent")
            ).scalar() or 0
            online = conn.execute(
                text(
...
def _register_builtin_tools(registry: ToolRegistry) -> None:
    ...
    registry.register(
        ToolDefinition(
            ...
            description="Get a high-level fleet summary: host count, agent count, online agents, active insight count.",
            risk_class=RiskClass.RC1,
    ...
    registry.register(
        ToolDefinition(
            ...
            description="List recent configuration drift detections, optionally filtered by time window.",
            risk_class=RiskClass.RC1,
            parameters={
                "type": "object",
                "properties": {
    ...
    registry.register(
        ToolDefinition(
            ...
            description="Fetch the latest snapshots for a specific host across all collected snapshot kinds (system_vitals, disk_state, security_posture, etc.).",
            risk_class=RiskClass.RC1,
            parameters={
                "type": "object",
                "properties": {
...
def _sync_search_knowledge_base(params: dict[str, Any]) -> dict[str, Any]:
    """Semantic search on the RAG knowledge base via pgvector."""
    ...
    try:
        with engine.connect() as conn:
            # Text-based search fallback (embedding search requires async LLM call)
            rows = conn.execute(text("""
                SELECT dc.chunk_id::text, dc.content,
                       dc.document_title, dc.metadata_jsonb
    ...
    try:
        ...
        with engine.connect() as conn:
            rows = conn.execute(text("""
                ...
                LIMIT :limit
            """), {"hc": host_code, "kind": kind, "limit": limit}).mappings().all()
            return {
                "snapshots": [
                    {
...
def _summarize_payload(payload: Any) -> dict[str, Any]:
    ...
    for key in ("cpu_times", "memory", "load_avg", "partitions", "containers",
                "ufw", "fail2ban", "iptables_rules"):
        if key in payload:
            val = payload[key]
            if isinstance(val, dict):
                summary[key] = {k: v for k, v in val.items() if not isinstance(v, (dict, list))}
        ...
        try:
            ...
            try:
                ...
                while _aio.get_event_loop().time() < deadline:
                    msg_data = await pubsub.get_message(
                        ignore_subscribe_messages=True, timeout=1.0,
                    )
                    if msg_data and msg_data["type"] == "message":
                        return _json.loads(msg_data["data"])
            finally:
...
def _register_phase4_tools(registry: ToolRegistry) -> None:
    ...
    registry.register(ToolDefinition(
        ...
        description="Time-series of snapshots for a host by kind (system_vitals, disk_state, etc.).",
        risk_class=RiskClass.RC1,
        parameters={
            "type": "object",
            "properties": {
                ...
                "snapshot_kind": {
                    "type": "string",
                    "description": "Snapshot kind (system_vitals, disk_state, docker_state, etc.).",
                },
    ...
    registry.register(ToolDefinition(
        ...
        description="Execute a read-only diagnostic command on a remote agent (df -h, ps, systemctl, etc.).",
        risk_class=RiskClass.RC1,
        parameters={
            "type": "object",
            "properties": {
                ...
                "command": {
                    "type": "string",
                    "description": "Diagnostic command (from allowlist: df -h, ps aux, systemctl, etc.).",
                },
            },
    ...
    registry.register(ToolDefinition(
        ...
        parameters={
            ...
            "properties": {
                "agent_id": {"type": "string", "description": _DESC_AGENT_ID},
                "path": {"type": "string", "description": "File path (e.g. /etc/ssh/sshd_config)."},
            },
...
def _register_phase5_tools(registry: ToolRegistry) -> None:
    ...
    registry.register(ToolDefinition(
        ...
        description="Restart a systemd service on a host (from allowlist only). Requires HITL approval.",
        risk_class=RiskClass.RC2,
        parameters={
            "type": "object",
            "properties": {
                ...
                "service": {
                    "type": "string",
                    "description": "Service name (internalcmdb-agent, nginx, haproxy, docker, fail2ban).",
                },
            },
    ...
    registry.register(ToolDefinition(
        ...
        description="Truncate/rotate runaway Docker container log files. Requires HITL approval.",
        risk_class=RiskClass.RC2,
        parameters={
            "type": "object",
            "properties": {
                "log_path": {"type": "string", "description": "Full path to the container log file."},
                "container_name": {"type": "string", "description": "Container name for audit trail."},
            },
            "required": ["log_path"],
        },
    ...
    async def _tool_edit_config(params: dict[str, Any]) -> dict[str, Any]:
        ...
        current = await _tool_remote_diagnostic({
            "agent_id": agent_id, "command": path,
        })
        return {
    ...
    registry.register(ToolDefinition(
        ...
        description="Generate a diff for a config file edit on a remote host. RC-3: requires HITL + double approval.",
        risk_class=RiskClass.RC3,
        parameters={
            "type": "object",
            "properties": {
                "agent_id": {"type": "string", "description": _DESC_AGENT_ID},
                "path": {"type": "string", "description": "Config file path (e.g. /etc/ssh/sshd_config)."},
    ...
    registry.register(ToolDefinition(
        ...
        description="Apply iptables/ufw rule changes with 5-minute auto-revert safety. RC-3: requires HITL + double approval.",
        risk_class=RiskClass.RC3,
        parameters={
            "type": "object",
            "properties": {
