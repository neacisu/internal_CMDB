...
import { Brain, Send, User, Sparkles, FileText, Wrench, Bot } from "lucide-react";
...
export default function CognitiveChatPage() {
  ...
  const sendMessage = async () => {
    ...
    try {
      if (agentMode) {
        const agentResult = await startAgentSession({ goal: q });
        const assistantMsg: ChatMessage = {
          id: crypto.randomUUID(),
          role: "assistant",
          content: agentResult.final_answer || agentResult.error || "Agent session completed.",
          tool_calls: agentResult.tool_calls as ToolCallDisplay[],
