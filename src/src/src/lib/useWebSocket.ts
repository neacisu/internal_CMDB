...
export function useWebSocket<T = unknown>(
  ...
  // Stable ref so ws.onclose can schedule reconnection without a forward reference
  // to the `connect` useCallback (which would create a TDZ violation per the
  ...
  }, [url, enabled, onMessage, onOpen, onClose, heartbeatIntervalMs, maxRetries, clearTimers]);

  // Keep connectRef in sync with the latest connect implementation so the
  // closure inside ws.onclose always calls the most-current version.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Use connectRef.current() so this effect does not directly call a function
    // that calls setState synchronously (react-hooks/set-state-in-effect rule).
