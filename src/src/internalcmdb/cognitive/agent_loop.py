...
class AgentLoop:
    ...
    _FAST_KEYWORDS: frozenset[str] = frozenset({
        "check", "status", "list", "query", "show", "get", "fetch",
        "health", "disk", "memory", "cpu", "uptime", "ping",
    })
    # Keywords that require the reasoning model (root cause, investigation)
    _REASONING_KEYWORDS: frozenset[str] = frozenset({
        "investigate", "root cause", "why", "analyze", "correlate",
        "debug", "diagnose", "remediate", "fix", "repair", "plan",
    })
        ...
        *,
        ...
        try:
            await self._reasoning_loop(
                session, llm, executor, messages, tools_schema, start_time,
            )
        except Exception as exc:
            session.status = "failed"
    ...
    async def _reasoning_loop(
        ...
        for iteration in range(1, self._max_iterations + 1):
            ...
            step_result = await self._run_iteration(
                session, llm, executor, messages, tools_schema, iteration,
            )
    ...
    async def _run_iteration(
        ...
        content, tool_calls, finish_reason, message = self._parse_llm_response(
            session, response, iteration,
        )
        if session.status != "running":
            return None
        ...
        if tool_calls:
            ...
            await self._execute_tool_calls(
                session, executor, messages, tool_calls, iteration,
            )
        else:
        ...
        try:
            if tools_schema:
                response = await llm.tool_call(
                    messages, tools_schema, model_name=self._model_name,
                )
            else:
            ...
            logger.warning(
                ...
                iteration, self._consecutive_llm_failures, _LLM_CONSECUTIVE_FAILURES, exc,
            )
            if self._consecutive_llm_failures >= _LLM_CONSECUTIVE_FAILURES:
                session.status = "completed"
                session.final_answer = (
    ...
    async def _execute_tool_calls(
        ...
        for tc in tool_calls:
            ...
            await self._execute_single_tool(
                session, executor, messages, tool_name, tool_args, tc_id, iteration,
            )
    ...
    async def _execute_single_tool(
        ...
        observe_content = json.dumps(
            exec_result.output if exec_result.success
                    ...
                    with engine.connect() as conn:
                        conn.execute(
                            ...
                            """),
                            {
                                ...
                                "answer": session.final_answer[:5000] if session.final_answer else None,
