...
class KnowledgeBaseIngestor:
    ...
    async def ingest_all(
        ...
        counts["services"] = await self._safe_ingest(
            "services", self._ingest_services, session, kb
        )
        counts["insights"] = await self._safe_ingest(
            "insights", self._ingest_insights, session, kb
        )
    ...
    async def _ingest_services(self, session: AsyncSession, kb: KnowledgeBase) -> int:
        ...
        for row in rows:
            ...
            content = (
                f"Service {row['name']} (code={row['service_code']}): {desc}"
                .strip().rstrip(":")
            )
            ids = await kb.embed_document(
                content,
