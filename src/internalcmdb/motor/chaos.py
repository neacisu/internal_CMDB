...
_PRODUCTION_TARGETS = frozenset({
    "prod", "production", "prod-gpu", "prod-db", "prod-api",
})
...
class ChaosEngine:
    ...
    async def run_experiment(
        ...
        *,
        ...
        if exp_def is None:
            return ChaosResult(
                experiment=experiment_name, target=target, success=False,
                observations=[
                    f"Unknown experiment: {experiment_name}",
                    f"Available: {', '.join(EXPERIMENTS.keys())}",
                ],
                dry_run=dry_run, state=ExperimentState.FAILED.value,
            )
        ...
        safety_block = self._pre_flight_safety_checks(
            exp_def, experiment_name, target, dry_run,
            allow_production, experiment_id, now,
        )
        ...
        blast_block = self._check_blast_radius_cap(
            blast_radius, experiment_name, target, dry_run, experiment_id, now,
        )
        ...
        logger.info(
            ...
            experiment_id, experiment_name, target, dry_run, len(blast_radius),
        )
        ...
        if not dry_run:
            ...
            if not hitl_approved:
                return ChaosResult(
                    ...
                    dry_run=False, experiment_id=experiment_id,
                    state=ExperimentState.PENDING.value, started_at=now,
                    data_safety_verified=data_safe,
                )

        result = ChaosResult(
            experiment=experiment_name, target=target, success=False,
            blast_radius=blast_radius, dry_run=dry_run,
            experiment_id=experiment_id,
            state=ExperimentState.RUNNING.value, started_at=now,
            data_safety_verified=data_safe,
        )
    ...
    def _pre_flight_safety_checks(
            ...
            for prod_pattern in _PRODUCTION_TARGETS:
                if prod_pattern in target_lower:
                    return ChaosResult(
                        experiment=experiment_name, target=target, success=False,
                        observations=[
                            f"BLOCKED: target '{target}' matches production pattern "
                            f"'{prod_pattern}'. Set allow_production=True to override.",
                        ],
                        dry_run=dry_run, experiment_id=experiment_id,
                        state=ExperimentState.FAILED.value, started_at=now,
                    )
        ...
        if target in self._host_locks:
            return ChaosResult(
                experiment=experiment_name, target=target, success=False,
                observations=[
                    f"BLOCKED: host '{target}' already targeted by "
                    f"experiment {self._host_locks[target]}.",
                ],
                dry_run=dry_run, experiment_id=experiment_id,
                state=ExperimentState.FAILED.value, started_at=now,
            )
    ...
    def abort(self, experiment_id: str) -> bool:
        ...
        if experiment_id in self._running:
            ...
            self._running[experiment_id].observations.append(
    ...
    def _calculate_blast_radius(
        self, exp_def: ExperimentDef, target: str
    ) -> list[str]:
    ...
    async def _check_hitl_approval(
        ...
        logger.info(
            "HITL approval required for experiment_id=%s: %s on %s "
            "(risk_class=%s, dual=%s)",
            experiment_id,
            exp_def.name,
            target,
    ...
    async def _simulate(
        ...
        result.observations.append(
            "Simulation complete — no real changes were made"
        )
        return result
    ...
    async def _execute_with_timeout(
        ...
        try:
            ...
            logger.error(
                ...
                result.experiment_id, exp_def.max_duration_seconds,
            )
            await self._execute_rollback(exp_def, target, result)
            return result
    ...
    async def _execute(
        ...
        while elapsed < exp_def.max_duration_seconds:
            if result.experiment_id in self._aborted:
                result.state = ExperimentState.ABORTED.value
                result.observations.append(
                    f"Execution aborted by kill switch at {elapsed:.1f}s"
                )
                logger.warning(
                    "Chaos experiment %s ABORTED at %.1fs",
                    result.experiment_id, elapsed,
                )
                await self._execute_rollback(exp_def, target, result)
                return result
        ...
        result.state = ExperimentState.COMPLETED.value
        result.observations.append(
            f"Recovery verification: {'PASS' if recovery_ok else 'FAIL'} "
    ...
    def _verify_data_safety(self, exp_def: ExperimentDef, target: str) -> bool:
        ...
        for pattern in data_critical_targets:
            if pattern in target_lower:
                logger.warning(
                    "Data safety BLOCKED for %s on %s: target matches data-critical "
                    "pattern '%s'", exp_def.name, target, pattern,
                )
                return False
    ...
    async def _execute_rollback(
        ...
        logger.warning(
            ...
            exp_def.name, target, result.experiment_id,
        )
