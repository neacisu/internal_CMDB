...
def _default_str_list() -> list[str]:
    ...
    Pylance ``reportUnknownVariableType`` — ``list`` alone yields ``list[Unknown]``
    because the builtin is not parameterised at call-site.
    """
    return []
...
class PlanTask:
    ...
    task_id: str
    description: str
    depends_on: list[str] = field(default_factory=_default_str_list)
    model_hint: str = "reasoning"
    status: str = "pending"  # pending, running, completed, failed, skipped
    result: str = ""


def _default_plan_task_list() -> list[PlanTask]:
    """Typed default factory for list[PlanTask] dataclass fields.

    Must be defined after :class:`PlanTask` so the return annotation resolves,
    and before :class:`ExecutionPlan` which uses it as a default_factory.
    """
    return []
...
class ExecutionPlan:
    ...
    plan_id: str = ""
    goal: str = ""
    tasks: list[PlanTask] = field(default_factory=_default_plan_task_list)
    summary: str = ""
    created_at: str = ""
