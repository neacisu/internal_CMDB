            ...
            try:
                with engine.connect() as conn:
                    ...
                    hs_rows = (
                        ...
                        LIMIT :limit
                    """),
                            {"limit": _MAX_INSIGHTS_TO_PROCESS},
                        )
                        .mappings()
                        .all()
                    )
                    ...
                    # Host health
                    host_count = (
                        conn.execute(text("SELECT COUNT(*) FROM registry.host")).scalar() or 0
                    )
                        ...
                        """),
                            {
                                "title": title,
                                "expl": (
                                    f"{rejected} tool executions were rejected by human"
                                    f" operators. Reasons: "
                                    f"{'; '.join(rejection_reasons[:3]) or 'not provided'}."
                                    " The autonomous loop will adjust confidence thresholds."
                                ),
