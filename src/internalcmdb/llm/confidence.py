...
_DOMAIN_KEYWORDS: dict[str, list[str]] = {
    "infrastructure": ["host", "server", "disk", "cpu", "memory", "network", "container", "service"],
    "security": ["vulnerability", "cve", "certificate", "tls", "ssh", "firewall", "credential", "pii"],
...
class ConfidenceCalibrator:
    ...
    async def calibrated_call(
        ...
        *,
        ...
        vq_messages = self._build_verification_question_messages(prompt, draft_content)
        try:
            if total_tokens >= _MAX_COVE_TOTAL_TOKENS:
                raise RuntimeError("CoVe token budget exhausted before verification")
            vq_resp = await self._call_llm(vq_messages, model)
            vq_content = self._extract_content(vq_resp)
            total_tokens += self._count_tokens(vq_resp)
            verification_questions = self._parse_questions(vq_content)
        except Exception:
            verification_questions = []

        # ── Step 4: Independent answers ──────────────────────────────────
        verification_steps: list[str] = []
        for vq in verification_questions[:_MAX_VERIFICATION_QUESTIONS]:
            if total_tokens >= _MAX_COVE_TOTAL_TOKENS:
                logger.warning(
                    "CoVe token budget exhausted (%d >= %d), stopping verification",
                    total_tokens, _MAX_COVE_TOTAL_TOKENS,
                )
                break
            va_messages = self._build_independent_answer_messages(vq, context)
            try:
                va_resp = await self._call_llm(va_messages, model)
                va_content = self._extract_content(va_resp)
                total_tokens += self._count_tokens(va_resp)
                verification_steps.append(f"Q: {vq} → A: {va_content[:200]}")
            except Exception:
                verification_steps.append(f"Q: {vq} → A: (verification failed)")

        # ── Step 5: Verified synthesis ───────────────────────────────────
        synth_messages = self._build_synthesis_messages(
            prompt, draft_content, verification_steps
        )
        try:
            if total_tokens >= _MAX_COVE_TOTAL_TOKENS:
                logger.warning("CoVe token budget exhausted, skipping synthesis")
                raise RuntimeError("token budget")
            synth_resp = await self._call_llm(synth_messages, model)
            final_content = self._extract_content(synth_resp)
            total_tokens += self._count_tokens(synth_resp)
        except Exception:
            final_content = draft_content

        raw_confidence = initial_confidence + 0.1 * len(verification_steps)
        final_confidence = self._clamp_confidence(raw_confidence)
        is_verified = len(verification_steps) > 0
        requires_hitl = final_confidence < threshold
    ...
    def _build_eval_messages(
        prompt: str, draft: str, domain: str
    ) -> list[dict[str, Any]]:
        return [
            {
                "role": "system",
    ...
    @staticmethod
    def _build_verification_question_messages(
        prompt: str, draft: str
    ) -> list[dict[str, Any]]:
        return [
            {
                "role": "system",
    ...
    @staticmethod
    def _build_independent_answer_messages(
    ...
    def _parse_confidence(eval_text: str) -> float:
        try:
            ...
            if isinstance(data, dict):
                raw = float(data.get("confidence", 0.5))
                return ConfidenceCalibrator._clamp_confidence(
                    raw if raw <= 1.0 else raw / 100.0
                )
    ...
    def _parse_questions(vq_text: str) -> list[str]:
        ...
        # JSON decode failures are JSONDecodeError → ValueError; avoid redundant except targets (S5713).
        except ValueError:
            pass
        lines = [ln.strip().lstrip("- ").lstrip("•").strip()
                 for ln in vq_text.strip().splitlines() if ln.strip()]
