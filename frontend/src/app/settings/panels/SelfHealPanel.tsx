...
export default function SelfHealPanel() {
  ...
  const [form, setForm] = useState<SelfHealConfig | null>(null);
        ...
        <CardContent className="flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-(--tx1) text-sm font-medium font-(--fM)">
            ...
            <input
              ...
              onChange={e => setForm(f => f ? { ...f, disk_threshold_pct: Number(e.target.value) } : f)}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-(--tx3) text-xs">
              ...
              Currently: <span className="font-(--fM) text-(--tx2)">{bytesToGb(form.log_auto_truncate_bytes)} GB</span>
            </p>
            <Input
              id="log-truncate"
              type="number"
              ...
              onChange={e => setForm(f => f ? { ...f, log_auto_truncate_bytes: Number(e.target.value) } : f)}
            />
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-(--tx3) text-xs">
              ...
              Currently: <span className="font-(--fM) text-(--tx2)">{bytesToGb(form.log_hitl_bytes)} GB</span>
            </p>
            <Input
              id="log-hitl"
              type="number"
              ...
              onChange={e => setForm(f => f ? { ...f, log_hitl_bytes: Number(e.target.value) } : f)}
            />
          </div>

          {validationError && (
            <div className="flex items-center gap-2 rounded-[8px] bg-(--er)/10 border border-(--er)/30 px-3 py-2">
              <AlertTriangle className="h-4 w-4 text-(--er) flex-shrink-0" />
