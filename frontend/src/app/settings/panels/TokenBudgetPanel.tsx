...
export default function TokenBudgetPanel() {
  ...
  useEffect(() => {
    if (data) {
      setRows(
        Object.fromEntries(
          data.map(b => [b.caller, { tokens_per_hour: b.tokens_per_hour, dirty: false }])
        )
      );
    }
  }, [data]);
  ...
  const saveMut = useMutation({
    ...
    onSuccess: (_data, vars) => {
      ...
      setRows(r => ({ ...r, [vars.caller]: { ...r[vars.caller], dirty: false } }));
  ...
  if (isLoading) return (
    <div className="flex flex-col gap-2">
      {[1, 2, 3, 4].map(i => <Skeleton key={i} className="h-10 w-full rounded-[8px]" />)}
    </div>
  );
